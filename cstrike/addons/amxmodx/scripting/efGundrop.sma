...
#define MODEL_GUNDROP			"models/ef_gundrop.mdl"
...
public plugin_init() 
{
	register_plugin("Addon Gundrop effect", "0.1", "PaXaN-ZOMBIE");
...
public FakeMeta_SetModel(const iEntity, const iModel[])
{
	...
	if(equal(iClassname, "weaponbox"))
	{
		...
		if (pev_valid(pEntity))
		{
			...
			set_pev(pEntity, pev_classname, "ef_gundrop");
...
public FakeMeta_Think(const iEntity)
{
	...
	if (equal(iClassname, "ef_gundrop"))
	{
