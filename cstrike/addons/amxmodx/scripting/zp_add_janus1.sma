...
new const GRENADE_EXPLOSION[] = "sprites/fire_cannon.spr"
new cvar_ammo_janus1
...
public plugin_init()
{
	...
	//register_clcmd("janus1","get_janus1")
}
...
{
	register_native("janus1","native_give_janus1",1)
}
public native_give_janus1(id)
{
	get_janus1(id)
}
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	else if(CurButton & IN_ATTACK2)
	{
		if(shoot_mode[id] == 2)
		{
			...
			shoot_mode[id] = 3
			peluru_hud(id)
			set_task(10.0, "back_normal", id)
			set_task(10.0, "back_normal2", id)
...
public Firejanus1(id)
{
	...
	velocity_by_aim( id, 3000, velocity )
...
public Damage_janus1(ptr, ptd)
{
	...
	if(!is_user_bot(Attacker) &&hit_janus1[Attacker] == 5 && shoot_mode[Attacker] == 1) set_task(1.0, "ready_transform", Attacker)
	if(is_user_bot(Attacker) && hit_janus1[Attacker] == 2 && shoot_mode[Attacker] == 1) set_task(1.0, "ready_transform", Attacker)
}
