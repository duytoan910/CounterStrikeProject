...
// Plugin Version
new const PLUGIN_VERSION[] = "Unlimited"
...
cvar_npc,cvar_npcchance,cvar_npcminplayers
...
new bought[33] // whether player is a bot
...
public plugin_natives()
{
	...
	register_native("zp_set_zombie_maxhealth", "native_set_zombie_maxhealth", 1)	
...
public plugin_precache()
{
	...
	defaultindex = precache_model("models/player/terror/terror.mdl")
...
public plugin_init()
{
	...
	for (new i = 1; i < sizeof WEAPONENTNAMES; i++)
		if (WEAPONENTNAMES[i][0]) RegisterHam(Ham_Item_AddToPlayer, WEAPONENTNAMES[i], "fw_Item_AddToPlayer", 1)
	...
	register_clcmd("zp_buy", "show_menu_extras")
	...
	// CVARS - Swarm Mode
	cvar_npc = register_cvar("zp_npc_enabled", "1")
	cvar_npcchance = register_cvar("zp_npc_chance", "20")
	cvar_npcminplayers = register_cvar("zp_npc_min_players", "0")
	...
	register_clcmd("say /extra","extras_list")
	g_item_human_count = 0;
}
...
public logevent_round_end()
{
	...
	if ((g_ambience_sounds[AMBIENCE_SOUNDS_NEMESIS] 
	&& g_nemround) || (g_ambience_sounds[AMBIENCE_SOUNDS_SURVIVOR] 
	&& g_survround) || (g_ambience_sounds[AMBIENCE_SOUNDS_SWARM] 
	&& g_swarmround) || (g_ambience_sounds[AMBIENCE_SOUNDS_PLAGUE] 
	&& g_plagueround) || (g_ambience_sounds[AMBIENCE_SOUNDS_INFECTION] 
	&& !g_nemround && !g_survround && !g_swarmround && !g_plagueround))
	{
...
public fw_PlayerSpawn_Post(id)
{
	...
	if (g_handle_models_on_separate_ent)
	{
		...
		if (get_pcvar_num(cvar_adminmodelshuman) && (get_user_flags(id) & g_access_flag[ACCESS_ADMIN_MODELS]))
		{
			...
			//if (g_set_modelindex_offset) fm_cs_set_user_model_index(id, defaultindex)
		}
		else
		{
...
public fw_PlayerKilled(victim, attacker, shouldgib)
{
	...
	if (get_pcvar_num(cvar_deathmatch))
	{
		...
		if(g_npcround)
			set_task(get_pcvar_float(cvar_spawndelay)*10.0, "respawn_player_task", victim+TASK_SPAWN)
		else set_task(get_pcvar_float(cvar_spawndelay), "respawn_player_task", victim+TASK_SPAWN)
	}
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	if (!g_zombie[attacker])
	{
		...
		{
			// Store damage dealt
			//g_damagedealt[attacker] += (floatround(damage)*0.09)
			// Reward ammo packs for every [ammo damage] dealt
			//while (g_damagedealt[attacker] > get_pcvar_num(cvar_ammodamage))
			//{
			new Float:multi
			if(g_isbot[attacker])
				multi=get_pcvar_float(cvar_ammodamage)*2
			else multi = get_pcvar_float(cvar_ammodamage)
	...
	if (g_survround || g_nemround || g_swarmround || g_plagueround || g_npcround || fnGetHumans() == 1)
...
public fw_Item_AddToPlayer(weapon_ent, id)
{
	// Get weapon's id
	static weaponid
	weaponid = cs_get_weapon_id(weapon_ent)
	...
	// Add to player's bpammo
	ExecuteHamB(Ham_GiveAmmo, id, 200, AMMOTYPE[weaponid], MAXBPAMMO[weaponid])
}
...
buy_extra_item(id, itemid, ignorecost = 0)
{
	...
	|| (g_survivor[id] && !(team & ZP_TEAM_SURVIVOR)) || g_npcround)
	{
		zp_colored_print(id, "^x04[ZP]^x01 %L", id, "CMD_NOT")
		return;
	}
	...
	if ((itemid == EXTRA_ANTIDOTE && (g_endround || g_swarmround || g_nemround || g_npcround || g_survround || g_plagueround || fnGetZombies() <= 1 || (get_pcvar_num(cvar_deathmatch) && !get_pcvar_num(cvar_respawnafterlast) && fnGetHumans() == 1)))
	|| (itemid == EXTRA_MADNESS && g_nodamage[id]) || (itemid == EXTRA_INFBOMB && (g_endround || g_swarmround || g_nemround || g_survround || g_plagueround || g_npcround)))
	{
...
make_a_zombie(mode, id)
{
	...
	if ((mode == MODE_NONE 
	&& (!get_pcvar_num(cvar_preventconsecutive) || g_lastmode != MODE_SURVIVOR) 
	&& random_num(1, get_pcvar_num(cvar_survchance)) == get_pcvar_num(cvar_surv) 
	&& iPlayersnum >= get_pcvar_num(cvar_survminplayers)) || mode == MODE_SURVIVOR)
	{
		// Survivor Mode
		g_survround = true
	...
	&& random_num(1, get_pcvar_num(cvar_npcchance)) == get_pcvar_num(cvar_npc) 
	&& iPlayersnum >= get_pcvar_num(cvar_npcminplayers)) || mode == MODE_NPC)
	{		
		// NPC Mode
		g_npcround = true
		g_lastmode = MODE_NPC
		...
		ShowSyncHudMsg(0, g_MsgSync, "%L", LANG_PLAYER, "NOTICE_NPC", g_playername[forward_id])
		
		// Mode fully started!
		g_modestarted = true
		
		// Round start forward
		ExecuteForward(g_fwRoundStart, g_fwDummyResult, MODE_NPC, forward_id);
	}
	...
	{		
		...
		for (id = 1; id <= g_maxplayers; id++)
		{
			...
			if(fm_cs_get_user_team(id)==FM_CS_TEAM_CT)
			{
				fm_set_user_health(id,floatround(get_pcvar_num(cvar_humanhp)*0.07))
			}
		}
	...
	&& iPlayersnum-(get_pcvar_num(cvar_plaguesurvnum)+get_pcvar_num(cvar_plaguenemnum)+floatround((iPlayersnum-(get_pcvar_num(cvar_plaguenemnum)+get_pcvar_num(cvar_plaguesurvnum)))*get_pcvar_float(cvar_plagueratio), floatround_ceil)) >= 1 && iPlayersnum >= get_pcvar_num(cvar_plagueminplayers)) || mode == MODE_PLAGUE)
	{
		...
		for (id = 1; id <= g_maxplayers; id++)
		{
			...
			if (fm_cs_get_user_team(id) != FM_CS_TEAM_CT) // need to change team?
			{
				remove_task(id+TASK_TEAM)
				fm_cs_set_user_team(id, FM_CS_TEAM_CT)
				fm_set_user_health(id,floatround(get_pcvar_num(cvar_humanhp)*0.1))
				fm_user_team_update(id)
			}
		}
	...
	for(i=0;i<get_maxplayers();i++)
	{
		...
		if (is_user_bot(i))
		{
			if (!bought[i])
			{
				...
				g_ammopacks[i] += random_num(-random(100)+1000,random(100)+1000)
			}
		}			
	}	
}
...
{	
	...
	if(g_isbot[id])
	{
		if (g_zombieclass[id] == ZCLASS_NONE) 
			g_zombieclass[id] = random_num(0, g_zclass_i - 1)
		g_zombieclassnext[id] = random_num(0, g_zclass_i - 1)
	}
	...
	if(!infector)
	{
		switch(random_num(1,30))
		{
			case 1..10:
			{
				engclient_cmd(id,"say", "What the fuck?")
			}
			case 11..20:
			{
				engclient_cmd(id,"say", "Wtf! Why me?")
			}
			case 21..30:
			{
				engclient_cmd(id,"say", "Dang it!")
			}
		}		
	}
	//nav_set_portrait(id)
...
public bot_buy_z(taskid)
{
	new id = ID_BOT_BUY_Z
	
	if(!is_user_alive(id) || !g_zombie[id] || g_nemround || g_survround || g_swarmround || g_plagueround || g_npcround)
		return;	
	...
	switch(num2)
	{
		...
		{
			static item_name[32]
			ArrayGetString(g_extraitem_name, temp, item_name, charsmax(item_name))
			engclient_cmd(id,"say", "I've just trying to buy",item_name)
		}
	}
...
humanme(id, survivor, silentmode)
{
	...
	else
	{
		...
		if(is_user_bot(id)&&!bought[id])
		{
			new par[1];
			par[0] = id;
			set_task(0.1,"bot_buy_extras_anti",12345,par,sizeof(par))
			client_print(id, print_chat, "Tasking!")
		}
	}
...
{	
	...
	for (i = 0; i < g_extraitem_i; i++)
	{
		...
		client_print(id, print_chat, "[%i] Name: %s, Price: %s",i,item_name,item_cost)
	}
...
public bot_buy_extras_anti(const args[])
{
	new id = args[0]
	if(g_zombie[id] || !g_isalive[id])
		return;
	
	client_print(id, print_chat, "Buy!")
...
public respawn_player_task(taskid)
{
	...
	if (!g_endround &&
	team != FM_CS_TEAM_SPECTATOR &&
	team != FM_CS_TEAM_UNASSIGNED &&
	!g_isalive[ID_SPAWN] &&
	...
	(!g_plagueround || get_pcvar_num(cvar_allowrespawnplague)))
	{
		...
		if (!get_pcvar_num(cvar_allowrespawninfection) && !g_survround && !g_nemround && !g_swarmround && !g_plagueround && !g_npcround)
			return;
		...
		else if (g_npcround) g_respawn_as_zombie[ID_SPAWN] = false
...
// Native: zp_set_zombie_maxhealth
public native_set_zombie_maxhealth(id, value)
{
	// ZP disabled
	if (!g_pluginenabled)
		return ;
	...
	if (g_zombie[id] && !g_nemesis[id])
	{
		ArraySetCell(g_zclass_hp, g_zombieclass[id],value)
	}
}
...
// Native: zp_is_npc_round
public native_is_npc_round()
{
	return g_npcround;
}
...
public native_get_zombie_class_id(const name[])
{
	...
	static i, class_name1[32]
	for (i = 0; i < g_zclass_i; i++)
	{
		ArrayGetString(g_zclass_name, i, class_name1, charsmax(class_name1))
		
		// Check if this is the class to retrieve
		if (equali(name, class_name1))
			return i;
	}
...
public set_user_nvision (taskid)
{
	...
	// Nemesis / Madness / nemesis in round Spectator
	...
	// Human / Spectator in normal
	else if (! g_zombie [ID_NVISION] ||! g_isalive [ID_NVISION])
	{
		write_byte (get_pcvar_num (cvar_humnvgcolor [0])) // r
		write_byte (get_pcvar_num (cvar_humnvgcolor [1]))
		write_byte (get_pcvar_num (cvar_humnvgcolor [2]))
		write_byte (70)
	}	
	// Zombie
	else
...
flare_lighting(entity, duration)
{
	...
	write_byte(color[0]-50) // r
	write_byte(color[1]-50) // g
	write_byte(color[2]-50) // b
	write_byte(51) //life
...
stock drop_weapons(id, dropwhat)
{
	...
	for (i = 0; i < num; i++)
	{
		...
		if ((dropwhat == 1 && ((1<<weaponid) & PRIMARY_WEAPONS_BIT_SUM)) || (dropwhat == 2 && ((1<<weaponid) & SECONDARY_WEAPONS_BIT_SUM)))
		{
			...
			set_pev(weapon_ent, PEV_ADDITIONAL_AMMO, 200)
