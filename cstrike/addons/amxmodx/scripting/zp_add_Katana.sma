...
#define PLUGIN "[ZP] Extra Item: Katana"
#define VERSION "1.0"
#define AUTHOR "https://fb.com/d.toan910"

#define V_MODEL "models/v_katana.mdl"
#define P_MODEL "models/p_katana.mdl"
...
#define CSW_KATANA CSW_KNIFE
#define weapon_katana "weapon_knife"
...
new const WeaponSounds[][] =
{
	...
	"weapons/katana_midslash1.wav",
	"weapons/katana_midslash1.wav",
	"weapons/katana_midslash2.wav",
	"weapons/katana_stap.wav",
	"weapons/katana_stapmiss.wav",
	"weapons/mastercombat_hit1.wav",
	"weapons/mastercombat_hit1.wav",
	"weapons/mastercombat_hit1.wav",
...
new g_Had_katana[33], g_Attack_Mode[33], g_Checking_Mode[33], g_Hit_Ing[33], g_SlashCount[33]
...
public plugin_init()
{
	...
	RegisterHam(Ham_Item_Deploy, weapon_katana, "fw_Item_Deploy_Post", 1)
	register_message(get_user_msgid("DeathMsg"), "Message_DeathMsg")
}

public plugin_natives()
{
	register_native("katana", "get_katana", 1)
}
...
public zp_user_infected_post(id) remove_katana(id)
public get_katana(id)
{
...
public Event_CurWeapon(id)
{
	...
	if((read_data(2) == CSW_KATANA && g_Old_Weapon[id] != CSW_KATANA) && g_Had_katana[id])
	{
		set_pev(id, pev_viewmodel2, V_MODEL)
		set_pev(id, pev_weaponmodel2, P_MODEL)
	
		set_weapon_anim(id, ANIM_DRAW)
		set_weapons_timeidle(id, CSW_KATANA, 1.0)
...
public fw_EmitSound(id, channel, const sample[], Float:volume, Float:attn, flags, pitch)
{
	...
	if(/*get_user_weapon(id) != CSW_KATANA || */!g_Had_katana[id])
		return FMRES_IGNORED
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	static ent; ent = fm_get_user_weapon_entity(id, CSW_KATANA)
	...
	if (CurButton & IN_ATTACK)
	{
		...
		remove_task(id+TASK_STABING)
		set_task(0.6, "Do_StabNow", id+TASK_STABING)
	}
}
public Primary_Attack(id)
{
	if(g_SlashCount[id] == 0)
	{
		g_SlashCount[id] = 1

		set_weapon_anim(id, ANIM_MSLASH2)
	}else{		
		g_SlashCount[id] = 0
		set_weapon_anim(id, ANIM_MSLASH1)
	}	
	set_weapons_timeidle(id, CSW_KATANA, 0.5)
...
public fw_PlayerTraceAttack(Victim, Attacker, Float:Damage, Float:Direction[3], TraceResult, DamageBits) 
{
	if(!is_user_alive(Attacker))	
		return HAM_IGNORED
	if(!g_Had_katana[Attacker] || !g_Checking_Mode[Attacker])
		return HAM_IGNORED
...
public Message_DeathMsg(msg_id, msg_dest, id)
{
	...
	if(get_user_weapon(iAttacker) == CSW_KATANA)
	{
		if(g_Had_katana[iAttacker])
