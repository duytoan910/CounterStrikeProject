...
#define MODEL_V "models/v_rpg.mdl"
#define MODEL_P "models/p_m3.mdl"
#define MODEL_W "models/w_m4a1.mdl"
#define MODEL_S "models/rpgrocket.mdl"
...
public plugin_init()
{
	...
	// g_at4s = zp_register_extra_item("Rocket",0,ZP_TEAM_HUMAN);
	// g_rocket = zg_weapon_register("Rocket", WPN_PRIMARY, CSW_AT4CS, 76,14)	
}
...
{
    register_native("Xoa_rocket", "native_Remove_rocket", 1)

}

public native_Remove_rocket(id)
{
       Remove_AT4CS(id)
}
...
public Get_AT4CS(id)
{
	...
	cs_set_user_bpammo(id, CSW_AT4CS, 70)
...
public fw_UpdateClientData_Post(id, sendweapons, cd_handle)
{
	...
	if(get_user_weapon(id) != CSW_AT4CS || !Get_BitVar(g_Had_AT4CS, id))
		return FMRES_IGNORED
...
public fw_PlaybackEvent(flags, invoker, eventid, Float:delay, Float:origin[3], Float:angles[3], Float:fparam1, Float:fparam2, iParam1, iParam2, bParam1, bParam2)
{
	if(!is_alive(invoker))
		return FMRES_IGNORED
	if(get_user_weapon(invoker) != CSW_AT4CS || !Get_BitVar(g_Had_AT4CS, invoker))
		return FMRES_IGNORED	
	if(eventid == g_WeaponEnt)
	{
		playback_event(flags | FEV_HOSTONLY, invoker, eventid, delay, origin, angles, fparam1, fparam2, iParam1, iParam2, bParam1, bParam2)	
		Set_WeaponAnim(invoker, 3)
...
{
	new ent, Float:Origin[3], Float:EndOrigin[3], Float:Angles[3], Float:VAngles[3], Float:Velocity[3]
	
	ent = engfunc(EngFunc_CreateNamedEntity, engfunc(EngFunc_AllocString, "info_target"))
	get_position(id, 20.0, 10.0, -10.0, Origin)
	get_position(id, 150.0, 10.0, 240.0, EndOrigin)
...
public setBeam(ent){
	...
	static Float:EndOrigin[3],Float:Cur[3],Float:Velocity[3]
	pev(ent, pev_origin, Cur)
	pev(ent, pev_iuser2, 1)
	get_position(ent, 2048.0, 0.0, 0.0, EndOrigin)
	get_speed_vector(Cur, EndOrigin, 600.0, Velocity)
	set_pev(ent, pev_velocity, Velocity)
...
public fw_Rocket_Think(Ent)
{
	...
	if(is_connected(Owner) && pev(Ent, pev_iuser1))
	{
		...
		if(is_user_alive(Victim)  && entity_range(Victim, Ent) <= 550.0)
		{
			...
			pev(Ent, pev_origin, Cur)
			get_speed_vector(Cur, Origin, 300.0, Velocity)
			set_pev(Ent, pev_velocity, Velocity)
		}
	}
...
public fw_Rocket_Touch(Ent, Id)
{
	...
	if(is_user_connected(Owner))
	{
		for(new i = 0; i < g_MaxPlayers; i++)
		{
			...
			if(get_user_weapon(Owner) != CSW_AT4CS || !Get_BitVar(g_Had_AT4CS, Owner))
				continue
...
public fw_TraceAttack(Ent, Attacker, Float:Damage, Float:Dir[3], ptr, DamageType)
{
	if(!is_alive(Attacker))
		return HAM_IGNORED
	if(get_user_weapon(Attacker) != CSW_AT4CS || !Get_BitVar(g_Had_AT4CS, Attacker))
		return HAM_IGNORED
...
public fw_Weapon_PrimaryAttack_Post(ent)
{
	...
	if(get_user_weapon(id) == CSW_AT4CS && Get_BitVar(g_Had_AT4CS, id) && cs_get_weapon_ammo(ent) > 0)
	{
		static Float:push[3]
