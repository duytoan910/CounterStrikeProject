...
new const g_grenade_description_h[][] = {
	" [Explosive Grenade]",
	" [Frost Grenade]",
	" [Flare Grenade]"
}
new const g_grenade_description_z[][] = {
	" [Infection Grenade]",
	" ",
	" [Frogbomb]"
}
...
public message_text(msgid, dest, id) {
	...
	for (new i; i < sizeof g_grenade_weaponid; ++i) {
		if (g_grenade_weaponid[i] == weapon) {
			...
			copy(text[++pos], sizeof text - pos - 1, zp_get_user_zombie(id)? g_grenade_description_z[i]:g_grenade_description_h[i])

			new desccolor = (i == 0 ? (zp_get_user_zombie(id) ? g_grenade_desccolor[2] : g_grenade_desccolor[0]) : g_grenade_desccolor[i])
