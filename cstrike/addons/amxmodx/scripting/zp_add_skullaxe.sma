...
#define V_MODEL "models/v_skullaxe.mdl"
#define P_MODEL "models/p_skullaxe.mdl"
...
#define SLASH_ROTATE_DAMAGE random_float(1700.0,1900.0)
...
#define STAB_DAMAGE random_float(1700.0, 1900.0)
#define STAB_RADIUS 140.0
...
{
    register_native("skullaxe","get_dragonsword", 1)
}
...
public Check_Attack(id, Mode)
{
	...
	for(new i = 0; i < get_maxplayers(); i++)
	{
		...
		|| get_distance_f(VicOrigin, Point[3]) <= Point_Dis)
		{
			...
			if(Mode == ATTACK_SLASH_ROTATE)
				do_attack(id, i, ent, is_deadlyshot(id)?SLASH_ROTATE_DAMAGE*1.5:SLASH_ROTATE_DAMAGE)
			else if(Mode == ATTACK_STAB)
				do_attack(id, i, ent, is_deadlyshot(id)?STAB_DAMAGE*1.5:STAB_DAMAGE)
		}
	}	
...
public Message_DeathMsg(msg_id, msg_dest, id)
{
	...
	if(get_user_weapon(iAttacker) == CSW_DRAGONSWORD)
	{
		if(g_Had_DragonSword[iAttacker])
