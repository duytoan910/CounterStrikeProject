...
// Variables
new g_iMaxPlayers, g_msgSayText, g_msgScreenFade, g_msgScreenShake,
...
public plugin_init()
{
	...
	cvar_duration = register_cvar("zp_siren_scream_duration", "1")
	cvar_screamdmg = register_cvar("zp_siren_scream_damage", "20")
	cvar_startime = register_cvar("zp_siren_scream_start_time", "1")
	cvar_reloadtime = register_cvar("zp_siren_scream_reload_time", "15.0")
	cvar_radius = register_cvar("zp_siren_scream_radius", "500.0")
...
public fw_Item_PostFrame(ent)
{
	...
	if(g_bCanDoScreams[id] && !g_bDoingScream[id] && !g_bRoundEnding){
		...
		set_task(0.1, "task_do_scream", id+TASK_SCREAM)
	}
}
...
public fw_CmdStart(id, handle, random_seed)
{
	...
	if(iInUseButton)
	{
		...
		// client_print(id, print_chat, "iInUseOldButton! %i", iInUseOldButton)
		// client_print(id, print_chat, "g_bCanDoScreams! %i", g_bCanDoScreams[id])
		// client_print(id, print_chat, "g_bDoingScream! %i", g_bDoingScream[id])
...
public task_reload_scream(id)
{
	...
	// Message
	client_print(id,print_center, "Ready, you can do /gScreams/y again")
}
	...
	while((iVictim = find_ent_in_sphere(iVictim, flOrigin, g_flCvar_Radius)) != 0)
	{
		...
		switch(g_iCvar_ScreamMode)
		{
			...
			case 0:
			{
				...
				// Do fake damage
				ExecuteHam(Ham_TakeDamage, iVictim, id, id, float(g_iCvar_ScreamDmg), DMG_BLAST)		
			}
