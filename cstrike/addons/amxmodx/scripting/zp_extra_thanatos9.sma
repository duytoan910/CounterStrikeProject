...
#define PLUGIN "[ZP][CSO] Thanatos9"
#define VERSION "1.0"
#define AUTHOR ""
...
------------ Configs -------------
=================================*/
#define DAMAGE_thanatos9 1146
#define DAMAGE_ATTACK 568

#define RADIUS_NORMAL 120
#define RADIUS_TMODE 150
...
#define V_MODEL "models/v_thanatos9.mdl"
#define P_MODEL_A "models/p_thanatos9a.mdl"
#define P_MODEL_B "models/p_thanatos9b.mdl"

new const Thanatos9_Sounds[11][] =
{
	...
	"weapons/thanatos9_shootb_end.wav", // 10
...
// Vars
new g_Thanatos9
new g_Had_thanatos9, g_NormalMode, g_TempingAttack, g_InSpecialAttack
new g_OldWeapon[33], g_WillBeHit[33], g_AttackingMode[33]
new g_MaxPlayers, g_Ham_Bot, mode2[33]
...
public plugin_init() 
{
	...
	register_clcmd("asdasdzxc", "Get_thanatos9") // tks anh neu anh khong xoa dong nay <3 :)))
}
public plugin_precache()
{
	engfunc(EngFunc_PrecacheModel, V_MODEL)
	engfunc(EngFunc_PrecacheModel, P_MODEL_A)
	engfunc(EngFunc_PrecacheModel, P_MODEL_B)
	
	for(new i = 0; i < sizeof(Thanatos9_Sounds); i++)
		engfunc(EngFunc_PrecacheSound, Thanatos9_Sounds[i])
		
	g_Thanatos9 = zp_register_extra_item("Thanatos-9", 15, ZP_TEAM_HUMAN)
}
...
public Get_thanatos9(id)
{
	...
	fm_give_item(id, weapon_thanatos9)
	if(get_user_weapon(id) == CSW_THANATOS9)
	{
		set_pev(id, pev_viewmodel2, V_MODEL)
		set_pev(id, pev_weaponmodel2, Get_BitVar(g_NormalMode, id) ? P_MODEL_A : P_MODEL_B)	

		set_player_nextattack(id, DRAW_TIME)
		set_weapons_timeidle(id, CSW_THANATOS9, DRAW_TIME + 0.5)
		
		set_weapon_anim(id, T9_ANIM_DRAWA)
	} else {
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSW_THANATOS9 && g_OldWeapon[id] != CSW_THANATOS9) && Get_BitVar(g_Had_thanatos9, id))
	{
		set_pev(id, pev_viewmodel2, V_MODEL)
		set_pev(id, pev_weaponmodel2, Get_BitVar(g_NormalMode, id) ? P_MODEL_A : P_MODEL_B)
		...
		set_weapon_anim(id, Get_BitVar(g_NormalMode, id) ? T9_ANIM_DRAWA : T9_ANIM_DRAWB)
		
		set_pdata_string(id, (492) * 4, ANIMEXT1, -1 , 20)
	} else if(CSWID != CSW_THANATOS9 && g_OldWeapon[id] == CSW_THANATOS9) {
		g_AttackingMode[id] = 0
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	if(CurButton & IN_ATTACK)
	{
		...
		if(Get_BitVar(g_NormalMode, id))
		{
			if(!g_AttackingMode[id])
			{
				...
				set_weapons_timeidle(id, CSW_THANATOS9, 1.0)
				set_player_nextattack(id, 2.0)
				
				set_weapon_anim(id, T9_ANIM_SHOOTA1)
				emit_sound(id, CHAN_WEAPON, Thanatos9_Sounds[7], 1.0, ATTN_NORM, 0, PITCH_NORM)
				set_task(1.1, "Damage_Slash1", id+TASK_SLASH)
				
				g_AttackingMode[id] = 1
			} else {
				//set_pdata_string(id, (492) * 4, ANIMEXT2, -1 , 20)
				...
				set_weapon_anim(id, T9_ANIM_SHOOTA2)
				emit_sound(id, CHAN_WEAPON, Thanatos9_Sounds[8], 1.0, ATTN_NORM, 0, PITCH_NORM)
				set_task(1.1, "Damage_Slash2", id+TASK_SLASH)
				g_AttackingMode[id] = 0
			}
		} else {
			set_pdata_string(id, (492) * 4, ANIMEXT2, -1 , 20)
			...
			set_weapons_timeidle(id, CSW_THANATOS9, 1.0)
			set_player_nextattack(id, 10.0)
			set_weapon_anim(id, T9_ANIM_BSTART)
			
			set_task(0.5, "attack2",id+TASK_ATTACK)
			set_task(5.0, "stopattack",id+TASK_ATTACK)
	...
	if(CurButton & IN_ATTACK2) 
	{
		...
		if(!mode2[id])
		{
			set_weapon_anim(id, T9_ANIM_CHANGEA)
			set_player_nextattack(id, CHANGE_TIME)
			set_weapons_timeidle(id, CSW_THANATOS9, CHANGE_TIME)
			set_task(4.0 , "Turn_Complete", id+TASK_TURN)
		}
		else 
		set_weapon_anim(id, T9_ANIM_IDLEB)
	}
}
public attack2(id)
{
	id -= TASK_ATTACK
	set_weapon_anim(id, T9_ANIM_BLOOP)
	set_player_nextattack(id, 5.0)
	set_weapons_timeidle(id, CSW_THANATOS9, 6.0)
	set_task(0.1, "Damage_Attack",id+TASK_SLASH)
	set_task(0.1 , "attack2", id+TASK_ATTACK)
	emit_sound(id, CHAN_WEAPON, "weapons/knife_hitwall.wav", 1.0, ATTN_NORM, 0, PITCH_NORM)
}
public stopattack(id)
{
	...
	set_task(0.1 , "Turn_Complete", id+TASK_TURN)
	set_weapon_anim(id, T9_ANIM_CHANGEB)
	set_player_nextattack(id, 3.2)
	set_weapons_timeidle(id, CSW_THANATOS9, 3.2)
}
...
public Turn_Complete(id)
{
	...
	set_pev(id, pev_weaponmodel2, Get_BitVar(g_NormalMode, id) ? P_MODEL_A : P_MODEL_B)
	set_pdata_string(id, (492) * 4, Get_BitVar(g_NormalMode, id) ? ANIMEXT1 : ANIMEXT2, -1 , 20)
}
...
public Damage_Slash1(id)
{
	...
	CheckAttack(id, float(RADIUS_NORMAL), 48.0, float(DAMAGE_thanatos9))
}

public Damage_Slash2(id)
{
	id -= TASK_SLASH
	if(!is_user_alive(id)) 
		return
...
public Damage_Attack(id)
{
	...
	CheckAttack(id, float(RADIUS_TMODE), 48.0, float(DAMAGE_ATTACK))
}
...
public fw_TraceLine(Float:vector_start[3], Float:vector_end[3], ignored_monster, id, handle)
{
	...
	if(Get_BitVar(g_TempingAttack, id)) xs_vec_mul_scalar(v_forward, (Get_BitVar(g_NormalMode, id) ? float(RADIUS_NORMAL) : float(RADIUS_TMODE)), v_forward)
...
public fw_TraceAttack(Victim, Attacker, Float:Damage, Float:Direction[3], TraceResult, DamageBits) 
{
	if(!is_user_alive(Attacker))	
		return HAM_IGNORED
	if(!Get_BitVar(g_Had_thanatos9, Attacker) || !Get_BitVar(g_TempingAttack, Attacker))
		return HAM_IGNORED
...
public fw_Weapon_WeaponIdle_Post(ent)
{
	...
	{
		set_weapon_anim(id, Get_BitVar(g_NormalMode, id) ? T9_ANIM_IDLEA : T9_ANIM_IDLEB)
