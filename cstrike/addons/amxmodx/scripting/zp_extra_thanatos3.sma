...
#define DAMAGE 50 // 45 for zombie
#define DAMAGE_SCYTHE 10
#define CLIP 60
#define BPAMMO 200
...
new g_MsgCurWeapon, g_InTempingAttack,g_thanatos3, m_iBlood[2]
new g_Event_Thanatos3, g_ShellId, g_HamBot
 ...
 public Message_DeathMsg(msg_id, msg_dest, id)
{
	...
	if(get_user_weapon(iAttacker) == CSW_THANATOS3)
	{
		if(Get_BitVar(g_Had_Thanatos3, iAttacker))
			set_msg_arg_string(4, "thanatos3")
	}
...
{ 
	if(itemid == g_thanatos3) Get_Thanatos3(id) 
} 
...
public Shoot_Scyche(id, Level)
{
	...
	get_position(id, 48.0, -5.0, random_float(-5.0, 5.0), Origin[0]); Speed[0] = 1000.0
	get_position(id, 48.0, 5.0, random_float(-5.0, 5.0), Origin[1]); Speed[1] = 1000.0
	get_position(id, 48.0, -10.0, random_float(-5.0, 5.0), Origin[2]); Speed[2] = 1000.0
	get_position(id, 48.0, 10.0, random_float(-5.0, 5.0), Origin[3]); Speed[3] = 1000.0
	get_position(id, 48.0, -15.0, random_float(-5.0, 5.0), Origin[4]); Speed[4] = 1000.0
	get_position(id, 48.0, 15.0, random_float(-5.0, 5.0), Origin[5]); Speed[5] = 1000.0
	...
	for(new i = 0; i < LoopTime; i++)
		Create_Scythe(id, Origin[i], Target, Speed[0])
}
...
public fw_Scythe_Think(Ent)
{
	...
	if(is_user_alive(Target))
	{
		...
		ExecuteHamB(Ham_TakeDamage, Target, Owner, Owner, is_deadlyshot(Owner)?random_float(DAMAGE_SCYTHE-5.0,DAMAGE_SCYTHE+5.0)+1.5:random_float(DAMAGE_SCYTHE-5.0,DAMAGE_SCYTHE+5.0), DMG_BULLET)
		...
		while((a = find_ent_in_sphere(a, VTarget, 35.0)) != 0)
		{
			if (Owner == a)
				continue 
			...
			{
				ExecuteHamB(Ham_TakeDamage, a, Owner, Owner, is_deadlyshot(Owner)?random_float(DAMAGE_SCYTHE-5.0,DAMAGE_SCYTHE+5.0)*0.5*1.5:random_float(DAMAGE_SCYTHE-5.0,DAMAGE_SCYTHE+5.0)*0.5, DMG_BULLET)
			}
		}
	} else {
		if(Target)
...
public fw_Weapon_PrimaryAttack_Post(Ent)
{
	...
	if(is_user_bot(id))
	{
		if(g_Thanatos3_Stage[id] == STAGE_METATRON) Check_Scythe(id)
	}
