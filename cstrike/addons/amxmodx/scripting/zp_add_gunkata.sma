...
#define P_GUNKATA_W "models/p_gunkata.mdl"
...
#define MODEL_MF "sprites/muzzleflash2.spr"
...
#define RANGE_SPECIAL  200.0
#define ANGLE_SPECIAL  360.0
#define KNOCKBACK_SPECIAL 150.0
#define DAMAGE_SPECIAL 110.0
...
new g_special[33], g_switch[33]
new g_Had_Base, g_Clip[33], g_OldWeapon[33], Float:g_Recoil[33][3]
new g_Event_Base, g_SmokePuff_SprId, cvar_shadow
...
public plugin_init()
{
	...
	cvar_shadow = register_cvar("dbg_show_shadow", "1")
...
public plugin_natives(){
	register_native("gunkata", "Get_Base", 1)
}
...
public fw_Item_AddToPlayer_Post(Ent, id)
{
	...
	write_string(Get_BitVar(g_Had_Base, id) ? "weapon_gunkata" : weapon_gunkata)
	write_byte(8)
	write_byte(35)
	write_byte(-1)
	write_byte(-1)
...
public WE_GUNKATA(id, Ent, iButton)
{
	...
	new Float:fDelay; pev(Ent, pev_fuser2, fDelay)
	new iMode = Stock_Gunkata_Mode(Ent)
	
	if(fSound && fSound < get_gametime() && g_special[id] == 2)
	{
		Gunkata_Blast(id)
		KnifeAttack2(id, false, RANGE_SPECIAL, ANGLE_SPECIAL, 0.0, KNOCKBACK_SPECIAL) // only do knockbacks
	...
	if(fDelay && fDelay < get_gametime() && g_special[id] == 1)
	{
		MakeMuzzleFlash(id, Ent, 3)
		set_pev(Ent, pev_fuser2, 0.0)
	}
	
	if(is_user_bot(id) && iButton & IN_ATTACK && g_special[id] != 3)
	{
		new a = FM_NULLENT
		static Float:Origin[3]
		pev(id, pev_origin, Origin)
		while((a = find_ent_in_sphere(a, Origin, RANGE_SPECIAL - 20)) != 0)
		{
			if(id == a)
				continue 
			if(!is_user_alive(a))
			...
			if(iClip <= 0 && 0 < g_special[id] < 2)
			{
				...
				set_pdata_float(Ent, 46, 0.8, 4)
				set_pdata_float(Ent, 47, 0.8, 4)
				set_pdata_float(Ent, 48, 0.8, 4)
			...
			if(iClip > 3) set_pdata_int(Ent, 51, iClip -random_num(1,2), 4)
			else set_pdata_int(Ent, 51, iClip - 1, 4)
			set_pdata_float(Ent, 47, 0.1, 4)
			
			new iHit = KnifeAttack2(id, false, RANGE_SPECIAL, ANGLE_SPECIAL, is_deadlyshot(id)?random_float(DAMAGE_SPECIAL-10.0,DAMAGE_SPECIAL+10.0)*1.5:random_float(DAMAGE_SPECIAL-10.0,DAMAGE_SPECIAL+10.0), 1.2)
	...
	if(!is_user_bot(id) && iButton & IN_ATTACK2 && g_special[id] != 3)
	{
		if(iClip <= 0 && 0 < g_special[id] < 2)
		{
			set_pev(Ent, pev_fuser1, get_gametime() + 0.66)
		...
		new iHit = KnifeAttack2(id, false, RANGE_SPECIAL, ANGLE_SPECIAL, DAMAGE_SPECIAL, 1.2)
	...
	if((!(iButton & IN_ATTACK2) && g_special[id]) || g_special[id] == 3)
	{
		if(get_pdata_float(Ent, 46, 4) > 0.0)
			return
...
{
	if(iClip) MakeMuzzleFlash(id, ent)
...
stock MakeMuzzleFlash(id, ent, iAttachNum = 0)
{
	static iMuz, iMode
	iMode = Stock_Gunkata_Mode(ent)
	...
	if(iAttachNum) set_pev(iMuz, pev_body, iAttachNum)
	else set_pev(iMuz, pev_body, !iMode?3:1)
...
stock DBG_Specials(ent, id, seq, Float:TimeRemove)
{
	...
	new pEntity = Stock_CreateEntityBase(id, "info_target", MOVETYPE_FLY, V_GUNKATA, "dbg_entytyd", SOLID_NOT,0.01,vecOrigin,vecAngle)
...
stock KnifeAttack2(id, bStab, Float:flRange, Float:fAngle, Float:flDamage, Float:flKnockBack, iHitgroup = -1, bNoTraceCheck = 0)
{
	...
	while ((pEntity = engfunc(EngFunc_FindEntityInSphere, pEntity, vecOrigin, flRange + 50)) != 0)
	{
		if (!pev_valid(pEntity))
			continue;
		if (id == pEntity)
