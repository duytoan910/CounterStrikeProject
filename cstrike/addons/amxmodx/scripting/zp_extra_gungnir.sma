...
#define V_GUNGNIR_B "models/v_gungnirex.mdl"
...
// range
#define WRANGE_PLASMA 100.0
#define WRANGE_CHARGE 100.0
#define WRANGE_BEAM 75.0

// knockback
#define WKNOCK_CHARGE 50.0 // only charge
...
public Get_Base(id) {
	...
	g_skin[id] = random_num(0,1)
        ...
        engfunc(EngFunc_MessageBegin, MSG_ONE_UNRELIABLE, get_user_msgid("CurWeapon"), {
                0,
                0,
                0
        }, id)
...
public fw_SetModel(entity, model[]) {
        ...
        if (equal(model, MODEL_W_OLD)) {
                static weapon;
                weapon = find_ent_by_owner(-1, weapon_gungnir, entity)
		...
		if(Get_BitVar(g_Had_Base, iOwner))
		{
                        ...
                        set_pev(weapon, pev_iuser3, g_skin[iOwner])
...
public fw_Item_Deploy_Post(Ent) {
        ...
        set_pev(Id, pev_viewmodel2, g_skin[Id]?V_GUNGNIR_B:V_GUNGNIR)
...
public fw_Item_PostFrame(ent) {
        ...
        flNextAttack = get_pdata_float(id, 83, 5)
        static bpammo;
        bpammo = cs_get_user_bpammo(id, CSW_GUNGNIR)
...
public WE_GUNGNIR(id, iEnt, iClip, bpammo, iButton) {
        new Float: flNextPrimaryAttack, Float: fCurTime, iState, Float: fSound, iCharge, isModeA[32]
        ...
        if ((1 <= enemy <= 32) && zp_get_user_zombie(enemy) && is_user_bot(id)) {
                ...
                if (range > ELECTRO_RANGE) isModeA[id] = true
        }
        ...
        if ((!(iButton & IN_ATTACK2) && 0 < iCharge < 3) || (is_user_bot(id) && isModeA[id] == true && !(iButton & IN_ATTACK))
                ...
                Stock_GetSpeedVector(vOrigin, vecEnd2, iMax ? 1500.0 : 2200.0, vVec)
                ...
                engfunc(EngFunc_SetModel, pEntity, g_skin[id]?EXP_MODELS_EX[iMax ? 7 : 5]:EXP_MODELS[iMax ? 7 : 5]);
                engfunc(EngFunc_SetSize, pEntity, Float: {
                        -0.5,
                        -0.5,
                        0.5
                }, Float: {
                ...
                if (iMax) {
                        Stock_Get_Postion(id, 15.0, 7.0, 3.0, vOrigin)
                }
                ...
                set_pdata_float(iEnt, 46, iMax ? 3.13 : (is_user_bot(id) ? 1.0 : 0.5), 4)
                set_pdata_float(iEnt, 48, iMax ? 3.5 : (is_user_bot(id) ? 1.25 : 0.75), 4)
        ...
        if (iButton & IN_ATTACK && iClip) {
                ...
                if (iState == 1) {
                        ...
                        write_short(g_cache_light[g_skin[id]])
                        write_byte(0)		// byte (starting frame) 
                        write_byte(60)		// byte (frame rate in 0.1's) 
                        write_byte(1)		// byte (life in 0.1's) 
                        write_byte(55)		// byte (line width in 0.1's) 
                        write_byte(25)		// byte (noise amplitude in 0.01's) 
                        write_byte(g_skin[id] ? 255 : 255)// r
                        write_byte(g_skin[id] ? 255 : 255)// g
                        write_byte(g_skin[id] ? 255 : 255)// b
                        write_byte(255)		// byte (brightness)
                        write_byte(60)		// byte (scroll speed in 0.1's)
                        message_end()
					...
					if (pev_valid(pEntity)) {
						...
						g_iVic[id][k] = pEntity
                                ...
                                if (is_user_alive(g_iVic[id][k]) && can_damage(id, g_iVic[id][k]) && entity_range(id, g_iVic[id][k]) < ELECTRO_RANGE && !Stock_Blah(fOrigin, LOL[k], id)) {
                                        ...
                                        engfunc(EngFunc_WriteCoord, LOL[k][2] - 5.0)
                                        write_short(g_cache_hit[g_skin[id]])
                                        write_byte(4)
                                        write_byte(30)
                                        ...
                                        engfunc(EngFunc_WriteCoord, LOL[k][0])
                                        engfunc(EngFunc_WriteCoord, LOL[k][1])
                                        engfunc(EngFunc_WriteCoord, LOL[k][2]+10.0)
                                        engfunc(EngFunc_WriteCoord, fEnd[0])
                                        engfunc(EngFunc_WriteCoord, fEnd[1])
                                        engfunc(EngFunc_WriteCoord, fEnd[2])
                                        write_short(g_cache_light[g_skin[id]])
                                        ...
                                        ExecuteHamB(Ham_TakeDamage, g_iVic[id][k], id, id, is_deadlyshot(id)?random_float(ELECTRO_DAMAGE-10,ELECTRO_DAMAGE+10)*1.5:random_float(ELECTRO_DAMAGE-10,ELECTRO_DAMAGE+10), DMG_BULLET)
                                        if (1 <= g_iVic[id][k] <= 32) {
                                                new Float: vAttacker[3], Float: vVictim[3]
                                                pev(id, pev_origin, vAttacker)
                                                pev(g_iVic[id][k], pev_origin, vVictim)
                                                ...
                                                set_pev(g_iVic[id][k], pev_velocity, vVictim)

                                                SpawnBlood(LOL[k], get_pdata_int(g_iVic[id][k], 89), floatround(ELECTRO_DAMAGE))
                                        }
                                }
                        }
                }
        ...
        if (iButton & IN_ATTACK2 && iClip) {
                ...
                case 1: {
                        ...
                        set_pev(iEnt, pev_fuser2, fCurTime + 0.03)
...
public HamF_InfoTarget_Think(iEnt) {
        ...
        if (iState) {
                ...
                write_short(g_cache_beam[g_skin[iOwner]][iState - 1])
                write_byte(0) // byte (starting frame) 
                ...
                write_byte(150) // byte,byte,byte (color) (R)
                write_byte(150) // (G)
                write_byte(150) // (B)
                ...
                if (get_gametime() >= fDelay && pev(iEnt, pev_iuser4)) {
                        ...
                        engfunc(EngFunc_SetSize, iBall, Float: {
                                -50.0,
                                -50.0,
                                0.0
                        }, Float: {
...
public HamF_InfoTarget_Touch(iEnt, iPtd) {
        ...
        while ((pEntity = engfunc(EngFunc_FindEntityInSphere, pEntity, vecOri, fRng)) != 0) {
                ...
                if (pev_valid(pEntity)) {
                        ...
                        ExecuteHamB(Ham_TakeDamage, pEntity, iOwner, iOwner, is_deadlyshot(iOwner)?random_float(fDmg-10.0,fDmg+10.0)*1.5:random_float(fDmg-10.0,fDmg+10.0), DMG_BULLET)
                        Stock_Fake_KnockBack(iOwner, pEntity, fKnc)
                        if (is_user_alive(pEntity)) {
			
						new Float: tempOrigin[3]
						pev(pEntity, pev_origin, tempOrigin)
						SpawnBlood(tempOrigin, get_pdata_int(pEntity, 89), floatround(fDmg / 5.0))
			}
                }
        }
        ...
        if (iState < 2) {
                ...
                engfunc(EngFunc_WriteCoord, vecOri[1])
                engfunc(EngFunc_WriteCoord, vecOri[2])
                write_short(g_cache_exp[g_skin[iOwner]][iState])
                write_byte(10)
                write_byte(30)
                write_byte(TE_EXPLFLAG_NODLIGHTS | TE_EXPLFLAG_NOSOUND | TE_EXPLFLAG_NOPARTICLES)
                message_end()
...
stock Stock_GetSpeedVector(const Float: origin1[3],
        const Float: origin2[3], Float: speed, Float: new_velocity[3]) {
...
--------- END OF SAFETY  ---------
=================================*/
stock Stock_SetPlayerAnim(id,
        const AnimName[], Float: rate = 1.0) {
...
stock Set_WeaponAnim(id, anim, iCheck = 0) {
        ...
        message_begin(MSG_ONE_UNRELIABLE, SVC_WEAPONANIM, {
                0,
                0,
                0
        }, id)
...
public bool: valueinarray(val, arr[])
{
    new i;
    for(i = 0; i < 5 ; i++)
    {
