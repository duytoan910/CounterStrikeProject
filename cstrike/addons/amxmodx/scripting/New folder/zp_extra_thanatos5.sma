...
#define DAMAGE_A 58 // 58 for zombie
#define DAMAGE_B 300.0 // 300 for zombie

#define CLIP 60
#define SCYTHE_RADIUS 100.0
...
new g_Had_Thanatos5, g_Thanatos5_Clip[33], g_GrenadeMode, g_thanatos5, shoot_t5[33]
new g_MsgCurWeapon, g_MsgStatusIcon, g_MsgWeaponList, g_InTempingAttack
new g_Event_Thanatos5, g_ShellId, g_HamBot, g_Exp_SprID, g_Exp2_SprID
...
public Message_DeathMsg(msg_id, msg_dest, id)
{
	...
	if(get_user_weapon(iAttacker) == CSW_THANATOS5)
	{
		if(Get_BitVar(g_Had_Thanatos5, iAttacker))
			set_msg_arg_string(4, "thanatos5")
	}
...
public fw_PlaybackEvent(flags, invoker, eventid, Float:delay, Float:origin[3], Float:angles[3], Float:fparam1, Float:fparam2, iParam1, iParam2, bParam1, bParam2)
{
	...
	if(is_user_bot(invoker))
	{
		shoot_t5[invoker]++
		//client_print(invoker, print_chat,"%i shot",shoot_t5[invoker])
		if(shoot_t5[invoker] >= 100)
		{
			Shoot_Scythe(invoker)
			shoot_t5[invoker] = 0
		}
	}	
...
public Shoot_Scythe(id)
{
	...
	Set_WeaponIdleTime(id, CSW_THANATOS5, 1.0)
...
public fw_Scythe_Think(Ent)
{
	...
	if(Time <= get_gametime())
	{
		...
		emit_sound(Ent, CHAN_WEAPON, WeaponSounds[2], VOL_NORM, ATTN_NONE, 0, PITCH_NORM)
...
public Create_ScytheSystem(id, Ent, Next)
{
	...
	get_position(Ent, 200.0, 0.0, 200.0, Origin[0])
	get_position(Ent, -200.0, 0.0, 200.0, Origin[1])
	get_position(Ent, 0.0, -200.0, 200.0, Origin[2])
	get_position(Ent, 0.0, 200.0, 200.0, Origin[3])
...
{
	new a = FM_NULLENT	
	while((a = find_ent_in_sphere(a, Float:Origin, SCYTHE_RADIUS)) != 0)
	{
		if(!is_user_alive(a))
			continue 
		if (id == a)
		...
		{
			ExecuteHamB(Ham_TakeDamage, a, id, id, is_deadlyshot(id)?random_float(DAMAGE_B-10.0,DAMAGE_B+10.0)*1.5:random_float(DAMAGE_B-10.0,DAMAGE_B+10.0), DMG_BULLET)
		}
	}	
}
