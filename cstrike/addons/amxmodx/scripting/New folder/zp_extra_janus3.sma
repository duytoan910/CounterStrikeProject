...
public plugin_init()
{
	...
	item_janus = zp_register_extra_item("Janus-III", 30, ZP_TEAM_HUMAN)
...
public plugin_precache()
{
	...
	precache_model("sprites/custommuzzleflash/muz_m16a1.spr")
	g_iEntity = create_entity("info_target")
	entity_set_model(g_iEntity, "sprites/custommuzzleflash/muz_m16a1.spr")
...
public CurrentWeapon(id)
{
     ...
     static Float:iSpeed
     if(g_has_janus3[id])
          if(janus3_mode[id] != 3) iSpeed = 1.0
...
public fw_janus3_PrimaryAttack_Post(Weapon)
{
	...
	else
	{
		...
		set_weapons_timeidle(Player, CSW_GALIL, 0.1)
...
public set_mode(Player)
{
	...
	//janus3_mode[id] = 3
	janus3_mode[Player] = 3
	set_weapons_timeidle(Player, CSW_GALIL, 1.7)
	set_player_nextattackx(Player, 1.7)
	set_task(8.0, "back_normal", Player)
	set_task(8.0, "back_normal2", Player)
}
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	if(CurButton & IN_ATTACK2)
	{
		if(janus3_mode[id] == 2 && get_pdata_float(id, 83, 5) <= 0.0)
		{
			...
			set_task(8.0, "back_normal", id)
			set_task(8.0, "back_normal2", id)
		}
	}
}
