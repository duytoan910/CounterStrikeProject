...
{ 
	...
	if(g_bInThirdPerson[iPlayer] || FORCED) 
	{ 
		Set_CameraEnt(iPlayer) 
	} 
	else 
...
public FW_CameraThink(iEnt)
{
	...
	fCameraOrigin[0] = fPlayerOrigin[0] + (-vBack[0] * DISTANCE) 
	fCameraOrigin[1] = fPlayerOrigin[1] + (-vBack[1] * DISTANCE) 
	fCameraOrigin[2] = fPlayerOrigin[2] + (-vBack[2] * DISTANCE) 
